...
// MUST HAVE USER IN THE DATABASE IN ORDER THIS TO WORK
...
async function main() {
  try {
    ...
    if (users.length === 0) {
      console.log(
        "No users found in the datbase. Seed cannot assign authorId without existing users.",
      );
      process.exit(1);
    }
    ...
    //
    console.log(`Inserted ${SEED_COUNT} articles(s) into the datbase\n`);
